...
// File: contracts/wDSC_new_unflattenend.sol
...
contract WrappedDenario is ERC20, Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Internal function to clean up outdated operations and verify limits.
    ...
    function _cleanupOldOperations(Operation[] storage operations) private returns (uint256 cleanLength) {
        ...
        // Find all operations outside the time window
        while (i < operations.length && operations[i].timestamp < cutoffTime) {
            i++;
        }

        // If any operations need to be removed
        if (i > 0) {
            uint256 j = 0;
            // Move all valid operations to the start of the array
        ...
        require(cleanLength < MAX_OPERATIONS, "Too many operations in time window");
    }
